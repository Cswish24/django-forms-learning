...
class ReviewDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        favorite_id = request.session.get("favorite_review")    #.get will handle error if no session data exists yet
...
class AddFavoriteView(View):
    def post(self, request):
        ...
        # fav_review = Review.objects.get(pk=review_id) # only store primitive types, dicts, lists in sessions. no objects allowed
        request.session["favorite_review"] = review_id
        return HttpResponseRedirect("/detail/" + review_id)
